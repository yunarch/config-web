...
createBaseProgram()
  ...
  .description(`Analyzes your codebase to detect missing and disconnected MSW handlers by cross-referencing service usages generated by \`openapi-gen\`.
Reports missing handlers with file locations and suggested paths, and warns about handlers that exist but are not registered in the MSW setup.

Note: Handlers defined inside MSW \`.use()\` calls are excluded from analysis.
These are runtime overrides typically used for per-test network behavior (e.g. simulating error responses) and are not part of the base network description.`)
  .requiredOption(
    '--gen <path>',
