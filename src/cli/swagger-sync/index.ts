...
async function generateModels({
  ...
  if (includeMswUtils) {
    await Promise.all([
      ...
      runTask({
        name: 'Generating Swagger MSW http utility',
        command: async () => {
          const content = await readFile(
            './src/cli/swagger-sync-msw-http.ts',
            'utf8'
          );
          await writeFile(
            `${modelsFolder}/swagger-msw-http.ts`,
            `/* generated -- do not edit */\n/* eslint-disable -- Autogenerated file */\nimport type swaggerJson from './swagger';\n${content}`
              .replaceAll('SwaggerDocumentType extends OpenApi.Document,', '')
              .replaceAll('SwaggerDocumentType', 'typeof swaggerJson')
          );
        },
      }),
    ]);
...
async function run({
  ...
  if (!hasLocalSwaggerPath) {
    const dir = path.dirname(localSwaggerPath);
    if (!existsSync(dir)) {
      await runTask({
...
createBaseProgram()
  ...
  .option(
    '--include-msw-utils',
    'Include MSW utilities to type-safe MSW handlers against the swagger schema.'
  )
  .option(
    '--post-script <script>',
    ...
    async (options: {
      ...
      try {
        await run({
          ...
          includeMswUtils: options.includeMswUtils,
